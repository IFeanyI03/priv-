...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    // VAULT
    if (message.type === "CHECK_VAULT_STATUS") { checkVaultStatus().then(sendResponse); return true; }
    else if (message.type === "SETUP_VAULT") { setupVault(message.password).then(sendResponse); return true; }
    else if (message.type === "UNLOCK_VAULT") { unlockVault(message.password).then(sendResponse); return true; }
    else if (message.type === "LOCK_VAULT") { sessionKey = null; sendResponse({ success: true }); return true; }

    // DATA
    else if (message.type === "GET_DECRYPTED_CREDENTIALS") { getDecryptedCredentials().then(sendResponse); return true; }
    else if (message.type === "SAVE_PASSWORD") { handleSavePassword(message.data).then(sendResponse); return true; }
    else if (message.type === "SAVE_SHARE_ACCESS") { saveShareAccess(message.data).then(sendResponse); return true; }

    // SHARING
    else if (message.type === "CREATE_SHARE") { createShare(message.data).then(sendResponse); return true; }
    else if (message.type === "GET_MY_SHARES") { getMyShares().then(sendResponse); return true; }
    else if (message.type === "REVOKE_SHARE") { revokeShare(message.id).then(sendResponse); return true; }
    else if (message.type === "RESOLVE_SHARED_LINK") { resolveSharedLink(message.id, message.key).then(sendResponse); return true; }
...
async function getDecryptedCredentials() {
    ...
    if (!err1 && personal) {
        for (const item of personal) {
            ...
            } catch (e) { console.warn("Skipping item (bad key):", item.site); }
        }
    }

    // 2. Shared
    const { data: shared, error: err2 } = await supabaseClient.rpc("get_shared_items_for_user", { my_user_id: user.id });
    if (shared) {
...
async function saveShareAccess(data) {
    ...
    try {
        ...
        const { data: currentShare } = await supabaseClient.from('credential_shares').select('shared_to, recipient_metadata').eq('id', data.share_id).single();
        ...
        const { error } = await supabaseClient.from('credential_shares').update({ shared_to: updatedSharedTo, recipient_metadata: updatedMetadata }).eq('id', data.share_id);
        if (error) throw error;
        return { success: true };
...
async function createShare(item) {
    try {
        ...
        const payload = JSON.stringify({ s: item.site, u: item.username, p: item.password, c: item.color, i: item.logo || "" });
        const encryptedData = await encryptData(payload, key);
        const saltBase64 = arrayBufferToBase64(salt.buffer);

        const { data, error } = await supabaseClient.from('credential_shares').insert({
...
async function resolveSharedLink(shareId, linkPassword) {
    try {
        const { data, error } = await supabaseClient.from('credential_shares').select(`*, credentials(site, username)`).eq('id', shareId).single();
        ...
        return { success: true, data: { ...decrypted, s: data.credentials?.site || decrypted.s, u: data.credentials?.username || decrypted.u } };
